...
const STORE_KEY = 'jj-music.mock.v2';
const LATENCY = 180;
...
function seedAlbum(
  ...
  return tracks.map((t, i) => ({
    id: `song_seed_${album.replace(/\W+/g, '').toLowerCase()}_${i + 1}`,
    title: t.title,
    artist,
    album,
    genre,
    ...
    artworkUrl: null,
    mimeType: 'audio/mpeg',
    fileSize: Math.round(t.duration * 40_000),
    favorite: Boolean(t.favorite),
    playCount: t.plays ?? 0,
    uploadedAt,
    lastPlayedAt: t.played ?? null,
  }));
}

/**
 * Placeholder metadata only — no commercial music, and no audio files, ship
 * with this repository. See README, "Adding a test file you own".
 */
function seedStore(): Store {
  const songs: Song[] = [
    ...seedAlbum('Ada Vance', 'Night Ferry', 'Indie', 2021, daysAgo(41), [
      { title: 'Harbour Lights', duration: 214, track: 1, plays: 12, favorite: true, played: hoursAgo(3) },
      { title: 'Six Miles Out', duration: 258, track: 2, plays: 7, played: hoursAgo(4) },
      { title: 'Cold Deck', duration: 191, track: 3, plays: 3 },
      { title: 'Night Ferry', duration: 302, track: 4, plays: 19, favorite: true, played: hoursAgo(26) },
    ]),
    ...seedAlbum('The Hollow Coast', 'Copper Lines', 'Rock', 2019, daysAgo(88), [
      { title: 'Copper Lines', duration: 236, track: 1, plays: 9, played: hoursAgo(30) },
      { title: 'Two Rivers', duration: 274, track: 2, plays: 4 },
      { title: 'Wire and Bone', duration: 199, track: 3, plays: 1, favorite: true },
      { title: 'The Long Way Down', duration: 341, track: 4, plays: 6, played: daysAgo(4) },
    ]),
    ...seedAlbum('Marisol Reyes', 'Slow Tide', 'Jazz', 2023, daysAgo(9), [
      { title: 'Morning Set', duration: 288, track: 1, plays: 15, favorite: true, played: hoursAgo(1) },
      { title: 'Blue Hour', duration: 327, track: 2, plays: 11, played: hoursAgo(2) },
      { title: 'Slow Tide', duration: 402, track: 3, plays: 8, played: daysAgo(2) },
    ]),
    ...seedAlbum('Ada Vance', 'Field Notes', 'Ambient', 2018, daysAgo(120), [
      { title: 'Static Field', duration: 365, track: 1, plays: 2 },
      { title: 'Paper Birds', duration: 244, track: 2, plays: 5, played: daysAgo(6) },
      { title: 'Late Signal', duration: 411, track: 3, plays: 0 },
    ]),
  ];
  ...
  const playlists: Playlist[] = [
    {
      id: 'pl_seed_1',
      name: 'Late drive',
      description: 'For the last hour of a long road.',
      artworkUrl: null,
      songIds: [songs[3].id, songs[4].id, songs[7].id, songs[10].id],
      songCount: 4,
      duration: 0,
    ...
    },
    {
      ...
      description: '',
      artworkUrl: null,
      songIds: [songs[8].id, songs[9].id, songs[12].id],
      songCount: 3,
      duration: 0,
